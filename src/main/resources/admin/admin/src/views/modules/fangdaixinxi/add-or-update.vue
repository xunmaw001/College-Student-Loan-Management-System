<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="放贷编号" prop="fangdaibianhao">
            <el-input v-model="ruleForm.fangdaibianhao" 
                placeholder="放贷编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fangdaibianhao" label="放贷编号" prop="fangdaibianhao">
              <el-input v-model="ruleForm.fangdaibianhao" 
                placeholder="放贷编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="每月利息" prop="meiyuelixi">
            <el-input v-model="meiyuelixi"
                placeholder="每月利息" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.meiyuelixi" label="每月利息" prop="meiyuelixi">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="放贷时间" prop="fangdaishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.fangdaishijian" 
                type="datetime"
                :readonly="ro.fangdaishijian"
                placeholder="放贷时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fangdaishijian" label="放贷时间" prop="fangdaishijian">
              <el-input v-model="ruleForm.fangdaishijian" 
                placeholder="放贷时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="放贷进度" prop="fangdaijindu">
          <el-select :disabled="ro.fangdaijindu" v-model="ruleForm.fangdaijindu" placeholder="请选择放贷进度">
            <el-option
                v-for="(item,index) in fangdaijinduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="放贷进度" prop="fangdaijindu">
	      <el-input v-model="ruleForm.fangdaijindu"
                placeholder="放贷进度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  computed: {
    ...
    meiyuelixi:{
      get: function () {
        return 1*this.ruleForm.daikuanjine*this.ruleForm.nianlilv
      }
    },

  },
  created() {
	this.ruleForm.fangdaishijian = this.getCurDateTime()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='fangdaibianhao'){
            this.ruleForm.fangdaibianhao = obj[o];
	    this.ro.fangdaibianhao = true;
            continue;
          }
	    ...
	    this.ro.meiyuelixi = true;
          ...
          if(o=='fangdaishijian'){
            this.ruleForm.fangdaishijian = obj[o];
	    this.ro.fangdaishijian = true;
            continue;
          }
          if(o=='xuehao'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='fangdaijindu'){
            this.ruleForm.fangdaijindu = obj[o];
	    this.ro.fangdaijindu = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.fangdaijinduOptions = "进行中,已完成".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `fangdaixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    // 提交
    onSubmit() {
        this.ruleForm.meiyuelixi = this.meiyuelixi
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fangdaixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fangdaixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
