<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="贷款编号" prop="daikuanbianhao">
            <el-input v-model="ruleForm.daikuanbianhao" 
                placeholder="贷款编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.daikuanbianhao" label="贷款编号" prop="daikuanbianhao">
              <el-input v-model="ruleForm.daikuanbianhao" 
                placeholder="贷款编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="银行名称" prop="yinxingmingcheng">
          <el-select :disabled="ro.yinxingmingcheng" @change="yinxingmingchengChange" v-model="ruleForm.yinxingmingcheng" placeholder="请选择银行名称">
            <el-option
                v-for="(item,index) in yinxingmingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.yinxingmingcheng" label="银行名称" prop="yinxingmingcheng">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="银行电话" prop="yinxingdianhua">
          <el-input v-model="ruleForm.yinxingdianhua" 
              placeholder="银行电话" clearable  :readonly="ro.yinxingdianhua"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="银行电话" prop="yinxingdianhua">
              <el-input v-model="ruleForm.yinxingdianhua" 
                placeholder="银行电话" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="适用条件" prop="shiyongtiaojian">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="适用条件"
                  v-model="ruleForm.shiyongtiaojian" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shiyongtiaojian" label="适用条件" prop="shiyongtiaojian">
                    <span>{{ruleForm.shiyongtiaojian}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="还款流程" prop="haikuanliucheng">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.haikuanliucheng" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.haikuanliucheng" label="还款流程" prop="haikuanliucheng">
                    <span v-html="ruleForm.haikuanliucheng"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	yinxingdianhua : false,
	daikuanjine : false,
	haikuanfangshi : false,
	daikuanshizhang : false,
	nianlilv : false,
	shiyongtiaojian : false,
	haikuanliucheng : false,
      },
      ruleForm: {
        xuexiaomingcheng: '',
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yinxingmingcheng = true;
          ...
          if(o=='yinxingdianhua'){
            this.ruleForm.yinxingdianhua = obj[o];
	    this.ro.yinxingdianhua = true;
            continue;
          }
          if(o=='daikuanjine'){
	    ...
	    this.ro.nianlilv = true;
          ...
          if(o=='shiyongtiaojian'){
            this.ruleForm.shiyongtiaojian = obj[o];
	    this.ro.shiyongtiaojian = true;
            continue;
          }
          if(o=='haikuanliucheng'){
            this.ruleForm.haikuanliucheng = obj[o];
	    this.ro.haikuanliucheng = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.xuexiaomingcheng!=''&&json.xuexiaomingcheng){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yinxing/yinxingmingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yinxingmingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    yinxingmingchengChange () {
      this.$http({
        url: `follow/yinxing/yinxingmingcheng?columnValue=`+ this.ruleForm.yinxingmingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.yinxingdianhua){
                this.ruleForm.yinxingdianhua = data.data.yinxingdianhua
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.haikuanliucheng = this.ruleForm.haikuanliucheng.replace(reg,'../../../springbootw659v/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhuxuedaikuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhuxuedaikuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
